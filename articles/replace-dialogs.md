...
|再利用 <br/> (子ダイアログとして起動)   |  No     | 有効 <br/> (アプリ内の任意の画面に移動、異なるアプリを新しいウィンドウで起動)     |
|開始/終了時にワークフローを実行    |   有効      |  No <br/> (代わりにフローを使用)  |
|入力時にワークフローを実行    | 有効    | No <br/> (代わりにフローを使用)   |
|ページの切り替え時にワークフローを実行   |  有効       | No <br/> (代わりにフローを使用)    |
|URL を使用して開始  |   No      |  有効     |
|セッションのログ記録    |  有効       |  No     |
|SDK のサポート   |  有効     |  有効     |
